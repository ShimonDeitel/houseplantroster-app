...
final class HouseplantRosterUITests: XCTestCase {
    ...
    func testAddItemFlow() {
        ...
        nameField.tap()
        nameField.typeText("UI Test Plant")
        app.buttons["itemSaveButton"].tap()
        XCTAssertTrue(app.staticTexts["UI Test Plant"].waitForExistence(timeout: 2))
    }
    ...
    func testFreeLimitTriggersPaywall() {
        for i in 0..<10 {
            ...
            app.buttons["addItemButton"].tap()
            if app.staticTexts["HouseplantRoster Pro"].waitForExistence(timeout: 1) {
                break
            }
            let nameField = app.textFields["itemNameField"]
